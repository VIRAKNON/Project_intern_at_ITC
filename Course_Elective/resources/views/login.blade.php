...
<body>
    ...
    <form class="form_container" action="{{ route('login_submit') }}" method="POST">
        ...
        @csrf
          ...
          <input placeholder="Email" name="email" type="text" class="input_field" id="email_field" value="{{ old('email') }}" required>
        </div>
        <div class="input_container">
        @csrf
          ...
          <input placeholder="Password" name="password" type="password" class="input_field" id="password_field" value="{{ old('password') }}" required>
        </div>
